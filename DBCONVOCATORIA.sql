...
END; 
 ...
 -- CUANTAS PERSONAS RESERVARON UN VUELO
 CREATE OR REPLACE FUNCTION FN_PERSONASVUELO(codVuelo RESERVAS.VUE_CODIGO%TYPE)
 RETURN NUMBER
 IS 
    contador NUMBER;
 BEGIN
 SELECT COUNT(*) INTO contador FROM RESERVAS 
 WHERE RESERVAS.VUE_CODIGO = codVuelo 
 AND RESERVAS.STATES_ESTADODERESERVA = 'ES00';
 RETURN contador;
 END;
 SELECT VUE_CODIGO, FN_PERSONASVUELO(VUE_CODIGO) FROM VUELOS;
